...
function spawnWeapon(clickX, clickY, typeOverride = null) {
    ...
    if (!typeOverride) {
        ...
        else if (type === 'mysterybox') cd = mysteryboxCooldown;
    ...
    if (type === 'drill') {
        ...
        // Set cooldown
        drillCooldown = MAX_COOLDOWNS.drill;
        return;
    }
...
function executeSpawn(type, clickX, clickY) {
    ...
    const shakeIntensity = getConfigValue(`weapons.${type}.shakeIntensity`, type === 'missile' ? 4.05 : type === 'nuke' ? 9.6 : type === 'moon' ? 32 : type === 'comet' ? 8 : 21);
